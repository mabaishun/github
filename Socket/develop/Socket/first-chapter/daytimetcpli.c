...
int main(int argc,char **argv)
{
    ...
    struct sockaddr_in servaddr;//网际套接字结构
    ...
    servaddr.sin_port = htons(13);//端口号，htons：主机到网络短整数
    if(inet_pton(AF_INET, argv[1], &servaddr.sin_addr) <=0 )//inet_pton:呈现形式到数值，即点分十进制到网络能识别的数值转换
